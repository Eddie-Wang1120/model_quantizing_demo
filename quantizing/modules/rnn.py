...
__all__ = ['quantizingLSTMCell', 'quantizingLSTM', 'convert_model_to_quantizing_lstm']
...
class quantizingLSTMCell(nn.Module):
    """
    A single LSTM block.
    ...
    def __init__(self, input_size, hidden_size, bias=True):
        super(quantizingLSTMCell, self).__init__()
    ...
    def from_pytorch_impl(lstmcell: nn.LSTMCell):
        module = quantizingLSTMCell(input_size=lstmcell.input_size, hidden_size=lstmcell.hidden_size, bias=lstmcell.bias)
...
def process_sequence_wise(cell, x, h=None):
    ...
    Args:
         cell (quantizingLSTMCell): the cell.
...
class quantizingLSTM(nn.Module):
    """
    ...
    def __init__(self, input_size, hidden_size, num_layers, bias=True, batch_first=False,
        ...
        super(quantizingLSTM, self).__init__()
        if num_layers < 1:
    ...
    def _create_cells_list(self, hidden_size_scale=1):
        ...
        cells = nn.ModuleList([quantizingLSTMCell(self.input_size, self.hidden_size, self.bias)])
        for i in range(1, self.num_layers):
            cells.append(quantizingLSTMCell(hidden_size_scale * self.hidden_size, self.hidden_size, self.bias))
        return cells
    ...
    def from_pytorch_impl(lstm: nn.LSTM):
        ...
        module = quantizingLSTM(lstm.input_size, lstm.hidden_size, lstm.num_layers, bias=lstm.bias,
...
def convert_model_to_quantizing_lstm(model: nn.Module):
    """
    Replaces all `nn.LSTM`s and `nn.LSTMCell`s in the model with quantizing versions.
    Args:
        model (nn.Module): the model
    """
    if isinstance(model, nn.LSTMCell):
        return quantizingLSTMCell.from_pytorch_impl(model)
    if isinstance(model, nn.LSTM):
        return quantizingLSTM.from_pytorch_impl(model)
    for name, module in model.named_children():
        module = convert_model_to_quantizing_lstm(module)
