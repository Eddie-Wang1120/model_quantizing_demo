...
import torch
import quantizing
from quantizing.utils import normalize_module_name
...
def load_checkpoint(model, chkpt_file, optimizer=None,
    ...
    def _load_and_execute_thinning_recipes():
        ...
        if normalize_dataparallel_keys:
            model.thinning_recipes = [quantizing.get_normalized_recipe(recipe)
                                      for recipe in model.thinning_recipes]
        quantizing.execute_thinning_recipes_list(model,
    ...
    def _create_model_from_ckpt():
        try:
            return quantizing.models.create_model(False, checkpoint['dataset'], checkpoint['arch'],
