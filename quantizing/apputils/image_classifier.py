...
import quantizing.quantization as quantization
import quantizing.models as models
from quantizing.models import create_model
from quantizing.utils import float_range_argparse_checker as float_range
...
class ClassifierCompressor(object):
    ...
    def train_one_epoch(self, epoch, verbose=True):
        ...
        with collectors_context(self.activations_collectors["train"]) as collectors:
            ...
            if verbose:
                quantizing.log_weights_sparsity(self.model, epoch, [self.tflogger, self.pylogger])
            quantizing.log_activation_statistics(epoch, "train", loggers=[self.tflogger],
                                                collector=collectors["sparsity"])
            if self.args.masks_sparsity:
                msglogger.info(quantizing.masks_sparsity_tbl_summary(self.model, 
    ...
    def validate_one_epoch(self, epoch, verbose=True):
        ...
        with collectors_context(self.activations_collectors["valid"]) as collectors:
            ...
            quantizing.log_activation_statistics(epoch, "valid", loggers=[self.tflogger],
                                                collector=collectors["sparsity"])
        ...
        if verbose:
            ...
            OrderedDict([('Loss', vloss),
                         ('Top1', top1),
                         ('Top5', top5)]))
            quantizing.log_training_progress(stats, None, epoch, steps_completed=0,
...
def init_classifier_compression_arg_parser(include_ptq_lapq_args=False):
    ...
    parser = argparse.ArgumentParser(description='quantizing image classification model compression')
    ...
    quantizing.quantization.add_post_train_quant_args(parser, add_lapq_args=include_ptq_lapq_args)
    return parser
...
def train(train_loader, model, criterion, optimizer, epoch,
    ...
    def _log_training_progress():
        ...
        quantizing.log_training_progress(stats,
                                        params,
...
def test(test_loader, model, criterion, loggers=None, activations_collectors=None, args=None):
    ...
    with collectors_context(activations_collectors["test"]) as collectors:
        ...
        quantizing.log_activation_statistics(-1, "test", loggers, collector=collectors['sparsity'])
        save_collectors_data(collectors, msglogger.logdir)
...
def _validate(data_loader, model, criterion, loggers, args, epoch=-1):
    def _log_validation_progress():
        ...
        quantizing.log_training_progress(stats, None, epoch, steps_completed,
...
def _convert_ptq_to_pytorch(model, args):
    msglogger.info('Converting quantizing PTQ model to PyTorch quantization API')
    dummy_input = quantizing.get_dummy_input(input_shape=model.input_shape)
    model = quantization.convert_quantizing_ptq_model_to_pytorch(model, dummy_input, backend=args.qe_pytorch_backend)
...
def quantize_and_test_model(test_loader, model, criterion, args, loggers=None, scheduler=None, save_flag=True):
    ...
    if args.device == 'cpu':
        ...
        qe_model = quantizing.make_non_parallel_copy(model).cpu()
    else:
...
def acts_histogram_collection(model, criterion, loggers, args):
    ...
    model = quantizing.utils.make_non_parallel_copy(model)
