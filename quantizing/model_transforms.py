...
from quantizing.quantization.sim_bn_fold import SimulatedFoldedBatchNorm
import logging
...
def fold_batch_norms(model, dummy_input=None, adjacency_map=None, inference=True):
    ...
    def fold_bn(sequence):
        ...
        try:
            ...
            msglogger.debug("Can't fold, {} does not track running stats".format(bn_module.quantizing_name))
            return None
        if inference:
...
def _fuse_sequence(sequence, named_modules, fuse_fn):
    names = [m.quantizing_name for m in sequence]
