...
def need_reduce_range(quantizing_quant_mode, torch_dtype):
    return torch.backends.quantized.engine == 'fbgemm' and not(is_linear_quant_mode_symmetric(quantizing_quant_mode) and
                                                               torch_dtype == torch.quint8)


def quantizing_qparams_to_pytorch(scale, zp, num_bits, quantizing_mode, dest_dtype, reduce_range=False):
    """
    Convert quantization parameters (scale and zero-point) calculated by quantizing APIs to quantization parameters
    compatible with PyTorch quantization APIs.

    By "calculated with quantizing APIs" we mean calculated using either of:
      * quantizing.quantization.symmetric_linear_quantization_params
      * quantizing.quantization.asymmetric_linear_quantization_params

    The main differences between quantization parameters as calculated by quantizing and PyTorch:
      * pytorch_scale = 1 / quantizing_scale
      * pytorch_zero_point = -quantizing_zero_point

    Args:
        scale (torch.Tensor): Scale factor calcualted by quantizing
        zp (torch.Tensor): Zero point calcualted by quantizing
        num_bits (int): Number of bits used for quantization in quantizing
        quantizing_mode (quantizing.quantization.LinearQuantMode): The quantization mode used in quantizing
    ...
    quantizing_symmetric = is_linear_quant_mode_symmetric(quantizing_mode)
    if quantizing_symmetric and dest_dtype == torch.quint8:
        reduce_range = False

    quantizing_asym_signed = quantizing_mode == LinearQuantMode.ASYMMETRIC_SIGNED
    ...
    if reduce_range:
        ...
        num_bits = 7
        if quantizing_symmetric:
            ratio = 63. / 127.
        else:
            ratio = 127. / 255.
            zp_offset = 128 if quantizing_asym_signed else 0
    ...
    # quantizing scale is the reciprocal of PyTorch scale
    scale_torch = 1. / scale

    n_bins_half = 2 ** (num_bits - 1)

    if quantizing_symmetric:
        # In quantizing symmetric is always signed with zero-point = 0, but in PyTorch it can be
        ...
        if quantizing_asym_signed and not pytorch_signed:
            zp = zp - n_bins_half
        elif not quantizing_asym_signed and pytorch_signed:
            zp = zp + n_bins_half
        # quantizing subtracts the zero-point when quantizing, PyTorch adds it.
        # So we negate the zero-point calculated in quantizing
        zp_torch = -zp
    return scale_torch, zp_torch


def quantizing_quantized_tensor_to_pytorch(tensor: torch.Tensor, scale, zp, num_bits, quantizing_mode, dest_dtype,
                                          per_channel=False, channel_dim=0):
    """
    Convert a tensor quantized with quantization parameters calculated by quantizing to a PyTorch "native" quantized
    tensor.
    ...
    And to tensors quantized using either of:
      * quantizing.quantization.linear_quantize
      * quantizing.quantization.linear_quantize_clamp

    Args:
        tensor (torch.Tensor): The tensor quantized in quantizing
        scale (torch.Tensor): Scale factor calcualted by quantizing
        zp (torch.Tensor): Zero point calcualted by quantizing
        num_bits (int): Number of bits used for quantization in quantizing
        quantizing_mode (quantizing.quantization.LinearQuantMode): The quantization mode used in quantizing
    ...
    converted_scale, converted_zp = quantizing_qparams_to_pytorch(scale, zp, num_bits, quantizing_mode, dest_dtype,
                                                                 reduce_range=False)
...
def _ptq_convert_pass_replace_range_linear_wrappers(module):
    ...
    # TODO: Proper fix
    from quantizing.quantization import RangeLinearQuantWrapper

    reassign = OrderedDict()
    for n, m in module.named_children():
        new_m = m
        if isinstance(m, quantizing.quantization.RangeLinearQuantWrapper):
            ...
            if requires_quantized_inputs:
                ...
                for idx, qmd in m.inputs_quant_metadata_fallback.items():
                    ...
                    scale, zp = quantizing_qparams_to_pytorch(qmd.scale, qmd.zero_point, qset.num_bits,
...
def convert_quantizing_ptq_model_to_pytorch(model, dummy_input, backend='fbgemm'):
    """
    ...
    Conversion is done in 2 passes:
      * First pass: Replace all RangeLinearQuantWrapper modules with a quantize operation followed by the respective
        native PyTorch module. Modules that weren't quantized by quantizing are wrapped with a de-quantize operation.
    ...
    # Hacky deferred import for now to workaround circular dependency
    # TODO: Proper fix
    from quantizing.quantization import PostTrainLinearQuantizer
    if not hasattr(model, 'quantizer_metadata') or model.quantizer_metadata['type'] != PostTrainLinearQuantizer:
        raise ValueError('Conversion to PyTorch native quantization supported only for models quantized '
                         'using quantizing.quantization.PostTrainLinearQuantizer')
    ...
    model = quantizing.make_non_parallel_copy(model).cpu()
