...
from quantizing.quantization import is_linear_quant_mode_asymmetric, is_linear_quant_mode_symmetric
...
def _make_non_parallel_copy(model):
    if any([isinstance(m, nn.DataParallel) for m in model.modules()]):
        return quantizing.make_non_parallel_copy(model)
    return model
...
def init_layer_linear_quant_params(quantizer, original_model, layer_name, init_mode=ClipMode.NONE,
    ...
    Args:
        ...
        search_clipping (bool): if set, optimize clipping values, otherwise optimize scale factor
    """
    denorm_layer_name = quantizing.denormalize_module_name(quantizer.model, layer_name)
    ...
    quantizing.model_setattr(quantizer.model, denorm_layer_name, quantized_layer)
...
def init_linear_quant_params(quantizer, original_model, eval_fn, dummy_input, init_mode,
    ...
    for module_name in layers_topological_order:
        ...
        q_module = q_named_modules[quantizing.denormalize_module_name(quantizer.model, module_name)]
...
def ptq_coordinate_search(quantizer, dummy_input, eval_fn, test_fn=None, method='Powell',
    ...
    Args:
        quantizer (quantizing.quantization.PostTrainLinearQuantizer): A configured PostTrainLinearQuantizer object
    ...
    run_device = quantizing.model_device(quantizer.model)
