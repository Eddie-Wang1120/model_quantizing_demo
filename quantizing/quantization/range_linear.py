...
import quantizing
import quantizing.utils
from .quantizer import Quantizer, QBits
from .q_utils import *
from .sim_bn_fold import SimulatedFoldedBatchNorm
import quantizing.modules
import quantizing.model_transforms as mt
...
def _check_clipping_val(val, quant_mode, half_range):
    ...
    if isinstance(val, (tuple, list, np.ndarray, torch.Tensor)):
        assert all(quantizing.is_scalar(v) for v in val), 'Elements of the clipping value must be scalar-like.'
...
class RangeLinearQuantWrapper(nn.Module):
    ...
    Args:
        ...
        activation_stats (dict): Dict containing activation stats, used for static calculation of quantization
            parameters. Dict should be in the format exported by quantizing.data_loggers.QuantCalibrationStatsCollector.
        ...
        if hasattr(input, 'quant_metadata'):
            if idx in self.inputs_quant_settings_overrides:
                raise RuntimeError('<{}> Input {}: CONFLICT - Tensor has embedded quantization metadata AND user '
                                   'defined input quantization settings'.format(self.quantizing_name, idx))
            ...
            if idx not in self.inputs_quant_settings_overrides and not self.inputs_quant_auto_fallback:
                raise RuntimeError('<{}> Input {}: Expected tensor with embedded quantization metadata. Either:\n'
                                   ...
                                   '3. Set inputs_quant_auto_fallback'.format(self.quantizing_name, idx))
...
class RangeLinearQuantParamLayerWrapper(RangeLinearQuantWrapper):
    ...
    def _convert_to_pytorch_quant(self, reduce_range):
        ...
        q_weight = pytqc.quantizing_quantized_tensor_to_pytorch(wrapped.weight.clone().detach(),
        ...
        # With quantizing PTQ the bias is only quantized on the first forward - we do a crude check if it has
        ...
        out_scale, out_zp = pytqc.quantizing_qparams_to_pytorch(self.output_scale, self.output_zero_point,
...
class RangeLinearQuantMatmulWrapper(RangeLinearQuantWrapper):
    ...
    The mathematical calculation is:
        ...
        y_q = ------------------- * ((i1_q + zp_i1) * (i2_q + zp_i2) + zp_y
               scale_i1 * scale_i2
    Args:
        wrapped_module (quantizing.modules.Matmul or quantizing.modules.BatchMatmul): Module to be wrapped
        num_bits_acts (int): Number of bits used for inputs and output quantization
        num_bits_accum (int): Number of bits allocated for the accumulator of intermediate integer results
        mode (ModuleQuantMode / LinearQuantMode): Quantization mode to use (symmetric / asymmetric-signed/unsigned)
        clip_acts (ClipNode): See RangeLinearQuantWrapper
    ...
    def __init__(self, wrapped_module, num_bits_acts, num_bits_accum=32,
        ...
        if not isinstance(wrapped_module, (quantizing.modules.Matmul, quantizing.modules.BatchMatmul)):
    ...
    def _convert_to_pytorch_quant(self, reduce_range):
        # Convert activations qparams - required by PyTorch to be unsigned 8-bit (torch.quint8)
        scale, zp = pytqc.quantizing_qparams_to_pytorch(self.output_scale, self.output_zero_point,
                                                       self.output_quant_settings.num_bits,
                                                       self.output_quant_settings.quant_mode, torch.quint8,
                                                       reduce_range)
        modules = [self.wrapped_module, nnq.Quantize(float(scale), int(zp), torch.quint8)]
        if self.clip_half_range:
            # The scale factor calculated in quantizing already considers the ReLU, so it's OK to apply the
...
class RangeLinearQuantConcatWrapper(RangeLinearQuantWrapper):
    def __init__(self, wrapped_module, num_bits_acts, mode=LinearQuantMode.SYMMETRIC, clip_acts=ClipMode.NONE,
                 ...
                 input_overrides=None, inputs_quant_auto_fallback=False):
        if not isinstance(wrapped_module, quantizing.modules.Concat):
            raise ValueError(self.__class__.__name__ + ' can only wrap quantizing.modules.Concat modules')
...
class RangeLinearQuantEltwiseAddWrapper(RangeLinearQuantWrapper):
    def __init__(self, wrapped_module, num_bits_acts, mode=LinearQuantMode.SYMMETRIC, clip_acts=ClipMode.NONE,
                 activation_stats=None, clip_n_stds=None, clip_half_range=False, scale_approx_mult_bits=None,
                 input_overrides=None, inputs_quant_auto_fallback=False):
        if not isinstance(wrapped_module, quantizing.modules.EltwiseAdd):
            raise ValueError(self.__class__.__name__ + ' can only wrap quantizing.modules.EltwiseAdd modules')
...
class RangeLinearQuantEltwiseMultWrapper(RangeLinearQuantWrapper):
    def __init__(self, wrapped_module, num_bits_acts, mode=LinearQuantMode.SYMMETRIC, clip_acts=ClipMode.NONE,
                 activation_stats=None, clip_n_stds=None, clip_half_range=False, scale_approx_mult_bits=None,
                 input_overrides=None, inputs_quant_auto_fallback=False):
        if not isinstance(wrapped_module, quantizing.modules.EltwiseMult):
            raise ValueError(self.__class__.__name__ + ' can only wrap quantizing.modules.EltwiseMult modules')
...
class FPWrapper(nn.Module):
    ...
    def forward(self, *input):
        if self.convert_input:
            input = quantizing.convert_tensors_recursively_to(input, dtype=self.dtype)

        result = self.wrapped_module(*input)
        if self.return_fp32:
            return quantizing.convert_tensors_recursively_to(result, dtype=torch.float32)
...
class RangeLinearEmbeddingWrapper(nn.Module):
    ...
    def to_pytorch_quant(self, reduce_range):
        ...
        scale, zp = pytqc.quantizing_qparams_to_pytorch(self.w_scale, self.w_zero_point,
                                                       self.wts_quant_settings.num_bits,
...
class RangeLinearFakeQuantWrapper(RangeLinearQuantWrapper):
    ...
    def quantized_forward(self, *inputs_q):
        inputs_q = quantizing.convert_tensors_recursively_to(inputs_q, dtype=self.dtype)
        outputs = self.wrapped_module(*inputs_q)
        return quantizing.convert_tensors_recursively_to(outputs, dtype=self.dtype)
...
class PostTrainLinearQuantizer(Quantizer):
    ...
    The following modules / operations have dedicated implementations which consider quantization:
      ...
      * torch.nn.Linear
      * torch.nn.Embedding
      * quantizing.modules.Concat
      * quantizing.modules.EltwiseAdd
      * quantizing.modules.EltwiseMult
      * quantizing.modules.Matmul
      * quantizing.modules.BatchMatmul
    An existing module will likely need to be modified to use the 'quantizing.modules.*' modules. This needs to
    be done BEFORE creating the quantizer. See the docs for more details:
    https://intellabs.github.io/quantizing/prepare_model_quant.html
    ...
    Args:
        ...
        model_activation_stats (str / dict / OrderedDict): Either a path to activation stats YAML file, or a dictionary
            ...
            The dict should be in the format exported by quantizing.data_loggers.QuantCalibrationStatsCollector.
            If None then parameters are calculated dynamically.
        ...
        inputs_quant_auto_fallback (bool): Enabled by default.
            See <quantizing_root>/examples/post_train_quant/resnet18_imagenet_post_train_input_overrides.yaml
    ...
    def __init__(self, model, bits_activations=8, bits_parameters=8, bits_accum=32,
        ...
        if model_activation_stats is not None:
            if isinstance(model_activation_stats, str):
                ...
                with open(model_activation_stats, 'r') as stream:
                    model_activation_stats = quantizing.utils.yaml_ordered_load(stream)
        ...
        def replace_param_layer(module, name, qbits_map, per_channel_wts=per_channel_wts,
            ...
            norm_name = quantizing.utils.normalize_module_name(name)
        ...
        def replace_fake_quant(module, name, qbits_map, fp16=fp16,
            ...
            if quantizing.has_children(module):
                return module
        ...
        self.replacement_factory[quantizing.modules.Concat] = factory_concat
        self.replacement_factory[quantizing.modules.EltwiseAdd] = factory_eltwiseadd
        self.replacement_factory[quantizing.modules.EltwiseMult] = factory_eltwisemult
        self.replacement_factory[quantizing.modules.Matmul] = factory_matmul
        self.replacement_factory[quantizing.modules.BatchMatmul] = factory_matmul
    ...
    def set_clipping(self, name, val):
        ...
        """
        module_name = quantizing.param_name_2_module_name(name)
    ...
    def from_args(cls, model, args):
        ...
        if args.qe_config_file:
            return quantizing.config_component_from_file_by_class(model, args.qe_config_file,
    ...
    def save_per_layer_parameters(self, save_dir=''):
        ...
        for n in self.module_overrides_map:
            ...
            m = modules_dict[n]
            if quantizing.has_children(m) and not is_post_train_quant_wrapper(m, include_fpwrapper=False):
                continue
        ...
        quantizing.yaml_ordered_save(save_path, out)
    ...
    def prepare_model(self, dummy_input=None):
        ...
        self.has_bidi_quantizing_lstm = any(isinstance(m, quantizing.modules.quantizingLSTM) and m.bidirectional for
                                           _, m in self.model.named_modules())
        if self.has_bidi_quantizing_lstm:
            warnings.warn('Model contains a bidirectional quantizingLSTM module. '
                          'Automatic BN folding and statistics optimization based on tracing is not yet '
                          'supported for models containing such modules.\n'
                          'Will perform specific optimization for the quantizingLSTM modules, but any other potential '
    ...
    def _apply_bn_folding(self, dummy_input):
        ...
        summary_graph = quantizing.SummaryGraph(self.model, dummy_input)
        ...
        for n, m in named_modules.items():
            # Look for the mark left by quantizing.model_transforms.fold_batch_norms
            fused_modules = getattr(m, 'fused_modules', None)
            if fused_modules is None:
                continue
            folded_bn_module = quantizing.normalize_module_name(fused_modules[0])
            ...
            if folded_bn_stats is None:
                continue
            model_stats[quantizing.normalize_module_name(n)]['output'] = folded_bn_stats['output']
    ...
    def _apply_activation_stats_fusions(self):
        ...
        for n, m in named_modules.items():
            ...
            if qbits.acts is None:
                continue
            if (quantizing.has_children(m) and not isinstance(m, SimulatedFoldedBatchNorm))\
                    or n not in self.adjacency_map or len(self.adjacency_map[n].successors) != 1:
                continue
            successor = self.adjacency_map[n].successors[0]
            n = quantizing.normalize_module_name(n)
            m_stats = model_stats[n]

            succ_type = successor.type
            succ_stats = model_stats.get(quantizing.normalize_module_name(successor.name), None)
            if succ_type == 'Split':
    ...
    def _apply_bidi_quantizing_lstm_stats_fusion(self):
        quantizing_lstm_cells = [n for n, m in self.model.named_modules() if
                                isinstance(m, quantizing.modules.quantizingLSTMCell)]

        for name in quantizing_lstm_cells:
    ...
    def _post_prepare_model(self):
        m = self.model
        device = quantizing.model_device(m)
    ...
    def convert_to_pytorch(self, dummy_input, backend='fbgemm'):
        """
        Convert a model quantized using quantizing.quantization.PostTrainLinearQuantizer to model comprised solely of
        native PyTorch static post-training quantization modules and operators.

        This is a convenience wrapper around quantizing.quantization.convert_quantizing_ptq_model_to_pytorch
        See that function's documentation for more details.
        """
        if not self.prepared:
            raise RuntimeError("Must call prepare_model before attempting to convert to PyTorch")

        return pytqc.convert_quantizing_ptq_model_to_pytorch(self.model, dummy_input, backend=backend)
...
class NCFQuantAwareTrainQuantizer(QuantAwareTrainRangeLinearQuantizer):
    def __init__(self, model, optimizer=None, bits_activations=32, bits_weights=32, bits_bias=32,
        ...
        self.replacement_factory[quantizing.modules.EltwiseMult] = self.activation_replace_fn
        self.replacement_factory[quantizing.modules.Concat] = self.activation_replace_fn
