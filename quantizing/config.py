...
1. Directly create and program scheduling objects.  For example:
    ...
    pruner = quantizing.pruning.SparsityLevelParameterPruner(name='sensitivity', levels=sparsity_levels)
    policy = quantizing.PruningPolicy(pruner, pruner_args=None)
...
import quantizing
from quantizing.thinning import *
from quantizing.pruning import *
from quantizing.regularization import *
from quantizing.learning_rate import *
from quantizing.quantization import *
from quantizing.utils import filter_kwargs
...
def dict_config(model, optimizer, sched_dict, scheduler=None, resumed_epoch=None):
    ...
    if scheduler is None:
        scheduler = quantizing.CompressionScheduler(model)
    ...
    try:
        ...
        for policy_def in sched_dict['policies']:
            ...
            if 'pruner' in policy_def:
                ...
                policy = quantizing.PruningPolicy(pruner, args)
                ...
                if args is None:
                    policy = quantizing.RegularizationPolicy(regularizer)
                else:
                    policy = quantizing.RegularizationPolicy(regularizer, **args)
                ...
                policy = quantizing.QuantizationPolicy(quantizer)
        ...
        for policy_def in lr_policies:
            ...
            policy = quantizing.LRPolicy(lr_scheduler)
            add_policy_to_scheduler(policy, policy_def, scheduler)
...
def file_config(model, optimizer, filename, scheduler=None, resumed_epoch=None):
    ...
    with open(filename, 'r') as stream:
        ...
        try:
            sched_dict = quantizing.utils.yaml_ordered_load(stream)
...
def config_component_from_file_by_class(model, filename, class_name, **extra_args):
    with open(filename, 'r') as stream:
        ...
        try:
            config_dict = quantizing.utils.yaml_ordered_load(stream)
