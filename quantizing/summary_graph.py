...
class SummaryGraph(object):
    ...
    def __init__(self, model, dummy_input, apply_scope_name_workarounds=True):
        ...
        model_clone = quantizing.make_non_parallel_copy(model)

        # Switch all instances of torch.nn.ModuleList in the model to our quantizingModuleList
        # See documentation of _quantizingModuleList class for details on why this is done
        model_clone, converted_module_names_map = _to_quantizing_modulelist(model_clone)

        with torch.onnx.select_model_mode_for_export(model_clone, False):
            
            device = quantizing.model_device(model_clone)
            dummy_input = quantizing.convert_tensors_recursively_to(dummy_input, device=device)
            ...
            for node in graph.nodes():
                ...
                # Get name from before conversion to quantizingModuleList
                ...
                module_name = quantizing.denormalize_module_name(self._src_model, module_name)
        ...
        for op in self.ops.values():
            ...
            if op['type'] == 'Conv':
                ...
                try:
                    ...
                    msglogger.error("For details see https://github.com/IntelLabs/quantizing/issues/168")
                    op['attrs']['MACs'] = 0
            elif op['type'] == 'Gemm':
                conv_out = op['outputs'][0]
                conv_in = op['inputs'][0]
                try:
                    ...
                    msglogger.error("An input to a Convolutional layer is missing shape information.")
                    msglogger.error("For details see https://github.com/IntelLabs/quantizing/issues/360")
                    n_ifm = n_ofm = 0
    ...
    def find_op(self, lost_op_name):
        return self.ops.get(quantizing.normalize_module_name(lost_op_name), None)
    ...
    def predecessors(self, node, depth, done_list=None, denorm_names=True):
        ...
        if denorm_names:
            ret = [quantizing.denormalize_module_name(self._src_model, x) for x in ret]
        return ret
    ...
    def predecessors_f(self, node_name, predecessors_types, done_list=None, logging=None, denorm_names=True):
        ...
        """
        node_name = quantizing.normalize_module_name(node_name)
        ...
        if node_is_an_op:
            ...
            if node['type'] in predecessors_types and len(done_list) > 1:
                return [quantizing.denormalize_module_name(self._src_model, node_name) if denorm_names else node_name]
    ...
    def named_params_layers(self):
        for param_name, param in self._src_model.named_parameters():
            ...
            normalized_layer_name = quantizing.normalize_module_name(
                '.'.join(param_name.split('.')[:-1]))
            sgraph_layer_name = quantizing.denormalize_module_name(
    ...
    def _dedicated_module_check(self, n, dedicated_modules_only=False):
        ...
        return len(self.module_ops_map[module_name]) == 1 and not quantizing.has_children(module)
    ...
    def adjacency_map(self, dedicated_modules_only=False):
        ...
        for op_name, op in self.ops.items():
            ...
            def op_meta(n):
                return OpSimpleMetadata(quantizing.denormalize_module_name(self._src_model, n), self.ops[n]['type'])
...
class _quantizingModuleList(object):
    r"""A almost-drop-in replacement for torch.nn.ModuleList that results in full and unique scope-names when traced

    So why do we need this?
      Some flows in quantizing, such as modules fusion and "net-aware" quantization in PostTrainLinearQuantizer, rely
      ...
      executed creates a node in the trace, which has a "scope-name". quantizing then uses the "scope-name" to do a
    ...
    From here on, we refer to the module which contains the quantizingModuleList instance as the "parent module".

    Similarities to torch.nn.ModuleList:
      * A quantizingModuleList can be indexed like a regular Python list, but the modules it contains are properly
      ...
      * This means that in terms of accessing the modules and invoking them, quantizingModuleList behaves exactly the
        same as torch.nn.ModuleList. See the example below.

    Differences vs. torch.nn.ModuleList:
      * quantizingModuleList is NOT a sub-class of torch.nn.Module
      * This means that the modules in the list are NOT sub-modules of the list itself. They are registered as
        sub-modules of the "parent module". That is - the contents of a quantizingModuleList are "flattened" within the
        "parent module".
    ...
    instances with quantizingModuleLists. Once that is done, during model execution we expect that lists are being
    ...
    def _verify_on_insertion(self, module, idx):
        if isinstance(module, nn.ModuleList):
            module = _quantizingModuleList(self._name_for_idx(idx), self.parent_module, module)
        if isinstance(module, _quantizingModuleList):
            if module.parent_module != self.parent_module:
                raise ValueError("When nesting one quantizingModuleList within another, both must have the same "
                                 "'parent_module'")
        return module
    ...
    def append(self, module):
        ...
        if not isinstance(module, _quantizingModuleList):
            self.parent_module.add_module(self._name_for_idx(len(self)), module)
        self._modules.append(module)

    def extend(self, modules):
        if not isinstance(modules, Iterable):
            raise TypeError('quantizingModuleList.extend must be called with an iterable, but got ' +
...
def _to_quantizing_modulelist(model):
    """Replaces all instances of torch.nn.ModuleList in a model with quantizingModuleList instances
    ...
    def convert_container(container):
        ...
        for name, child in named_children.items():
            if isinstance(child, nn.ModuleList):
                child = _quantizingModuleList(name, container, child)
                to_check = child.modules()
            else:
                to_check = [child]
            setattr(container, name, child)
            for m in to_check:
                if isinstance(m, _quantizingModuleList):
                    continue
                if quantizing.has_children(m):
                    convert_container(m)
        return container
