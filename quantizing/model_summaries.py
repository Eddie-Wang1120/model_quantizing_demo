...
def model_summary(model, what, dataset=None, logdir=''):
    if what.startswith('png'):
        ...
        quantizing.log_weights_sparsity(model, -1, loggers=[pylogger, csvlogger])
    elif what == 'compute':
        try:
            dummy_input = quantizing.get_dummy_input(dataset, quantizing.model_device(model))
...
def weights_sparsity_summary(model, return_total_sparsity=False, param_dims=[2, 4]):
    ...
    for name, param in model.state_dict().items():
        ...
        if param.dim() in param_dims and any(type in name for type in ['weight', 'bias']):
            _density = quantizing.density(param)
            params_size += torch.numel(param)
            sparse_params_size += param.numel() * _density
            df.loc[len(df.index)] = ([
                name,
                quantizing.size_to_str(param.size()),
                torch.numel(param),
                int(_density * param.numel()),
                quantizing.sparsity_cols(param)*100,
                quantizing.sparsity_rows(param)*100,
                quantizing.sparsity_ch(param)*100,
                quantizing.sparsity_2D(param)*100,
                quantizing.sparsity_3D(param)*100,
                (1-_density)*100,
                param.std().item(),
                param.mean().item(),
                param.abs().mean().item()
    ...
    for name, param in model.state_dict().items():
        ...
        if param.dim() in param_dims and any(type in name for type in ['weight', 'bias']):
            ...
            if mask is None:
                ...
                _density = quantizing.density(mask)
            params_size += torch.numel(param)
            sparse_params_size += param.numel() * _density
...
def conv_visitor(self, input, output, df, model, memo):
    ...
    if self in memo:
        return
    weights_vol = quantizing.volume(self.weight)

    # Multiply-accumulate operations: MACs = volume(OFM) * (#IFM * K^2) / #Groups
    # Bias is ignored
    macs = (quantizing.volume(output) *
...
def fc_visitor(self, input, output, df, model, memo):
    ...
    # Bias is ignored
    weights_vol = macs = quantizing.volume(self.weight)
...
def module_visitor(self, input, output, df, model, weights_vol, macs, attrs=None):
    ...
    mod_name = quantizing.model_find_module_name(model, self)
    df.loc[len(df.index)] = ([mod_name, self.__class__.__name__,
                              attrs if attrs is not None else '',
                              quantizing.size_to_str(in_features_shape), quantizing.volume(input[0]),
                              quantizing.size_to_str(out_features_shape), quantizing.volume(output),
...
def model_performance_summary(model, dummy_input, batch_size=1):
    ...
    model = quantizing.make_non_parallel_copy(model)
    model.apply(install_perf_collector)
    # Now run the forward path and collect the data
    dummy_input = dummy_input.to(quantizing.model_device(model))
    model(dummy_input)
...
def create_png(sgraph, display_param_nodes=False, rankdir='TB', styles=None):
    ...
    def annotate_op_node(op):
        if op['type'] == 'Conv':
            return ["sh={}".format(quantizing.size2str(op['attrs']['kernel_shape'])),
    ...
    for id, param in sgraph.params.items():
        n_data = (id, str(quantizing.volume(param['shape'])), str(param['shape']))
...
def draw_img_classifier_to_file(model, png_fname, dataset=None, display_param_nodes=False,
    ...
    """
    dummy_input = quantizing.get_dummy_input(dataset=dataset,
                                            device=quantizing.model_device(model),
                                            input_shape=input_shape)
    try:
        non_para_model = quantizing.make_non_parallel_copy(model)
