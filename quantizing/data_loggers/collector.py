...
import quantizing
from quantizing.quantization.range_linear import is_post_train_quant_wrapper
from quantizing.quantization.pytorch_quant_conversion import QFunctionalWrapper
import numpy as np
...
class ActivationStatsCollector(object):
    ...
    * It can't access the activations of torch.Functions, only torch.Modules.
    ...
    def __init__(self, model, stat_name, classes):
        ...
        quantizing.utils.assign_layer_fq_names(model)
        ...
        self._dont_collect_list = [module.wrapped_module.quantizing_name for module in model.modules() if
    ...
    def _should_collect(self, module):
        if module.quantizing_name in self._dont_collect_list:
            return False
        ...
        if quantizing.has_children(module) and not (is_post_train_quant_wrapper(module) or
...
class SummaryActivationStatsCollector(ActivationStatsCollector):
        ...
        try:
            ...
            if "The expanded size of the tensor" in e.args[0]:
                raise ValueError("ActivationStatsCollector: a module ({} - {}) was encountered twice during model.apply().\n"
                                 ...
                                 format(module.quantizing_name, type(module)))
            else:
                msglogger.info("Exception in _activation_stats_cb: {} {}".format(module.quantizing_name, type(module)))
                raise
    ...
    def _start_counter(self, module):
        if not hasattr(module, self.stat_name):
            ...
            if hasattr(module, 'quantizing_name'):
                getattr(module, self.stat_name).name = module.quantizing_name
            else:
...
class RecordsActivationStatsCollector(ActivationStatsCollector):
    ...
    def _activation_stats_cb(self, module, inputs, output):
        ...
        module.statistics_records['shape'] = quantizing.size2str(output)
    ...
    def _collect_activations_stats(self, module, activation_stats, name=''):
        if hasattr(module, "statistics_records"):
            activation_stats[module.quantizing_name] = module.statistics_records
...
def _verify_no_dataparallel(model):
    if torch.nn.DataParallel in [type(m) for m in model.modules()]:
        raise ValueError('Model contains DataParallel modules, which can cause inaccurate stats collection. '
                         ...
                         'quantizing.utils.make_non_parallel_copy on the model before invoking the collector')
...
class QuantCalibrationStatsCollector(ActivationStatsCollector):
    ...
    def _activation_stats_cb(self, module, inputs, output):
        ...
        def update_record(record, tensor):
            ...
            if not record['shape']:
                record['shape'] = quantizing.size2str(tensor)
    ...
    def _collect_activations_stats(self, module, activation_stats, name=''):
        ...
        activation_stats[module.quantizing_name] = OrderedDict()
        if module.quant_stats.inputs:
            activation_stats[module.quantizing_name]['inputs'] = OrderedDict()
            for idx, sr in enumerate(module.quant_stats.inputs):
                activation_stats[module.quantizing_name]['inputs'][idx] = sr
        activation_stats[module.quantizing_name]['output'] = module.quant_stats.output
    ...
    def save(self, fname):
        ...
        records_dict = self.value()
        quantizing.yaml_ordered_save(fname, records_dict)
...
class ActivationHistogramsCollector(ActivationStatsCollector):
    ...
    def __init__(self, model, activation_stats, classes=None, nbins=2048,
        ...
        if isinstance(activation_stats, str):
            ...
            with open(activation_stats, 'r') as stream:
                activation_stats = quantizing.utils.yaml_ordered_load(stream)
    ...
    def _activation_stats_cb(self, module, inputs, output):
        ...
        with torch.no_grad():
            for idx, input in enumerate(inputs):
                stat_min, stat_max = self._get_min_max(module.quantizing_name, 'inputs', idx)
                curr_hist = get_hist(input, stat_min, stat_max)
                module.input_hists[idx] += curr_hist

            stat_min, stat_max = self._get_min_max(module.quantizing_name, 'output')
            curr_hist = get_hist(output, stat_min, stat_max)
            module.output_hist += curr_hist

    def _reset(self, module):
        num_inputs = len(self.act_stats[module.quantizing_name]['inputs'])
    ...
    def _collect_activations_stats(self, module, activation_stats, name=''):
        ...
        for idx, hist in enumerate(module.input_hists):
            inputs_od[idx] = get_hist_entry(*self._get_min_max(module.quantizing_name, 'inputs', idx),
                                            module.input_hists[idx])

        output_od = get_hist_entry(*self._get_min_max(module.quantizing_name, 'output'), module.output_hist)

        stats_od['inputs'] = inputs_od
        stats_od['output'] = output_od
        activation_stats[module.quantizing_name] = stats_od
...
class RawActivationsCollector(ActivationStatsCollector):
    ...
    def _collect_activations_stats(self, module, activation_stats, name=''):
        ...
        activation_stats[module.quantizing_name] = module.raw_outputs
