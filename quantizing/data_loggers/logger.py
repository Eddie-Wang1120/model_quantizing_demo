...
import tabulate
import quantizing
from quantizing.utils import density, sparsity, sparsity_2D, size_to_str, to_np, norm_filters
# TensorBoard logger
...
class PythonLogger(DataLogger):
    ...
    def log_training_progress(self, stats_dict, epoch, completed, total, freq):
        ...
        for name, val in stats_dict.items():
            if isinstance(val, int):
                log = log + '{name} {val}    '.format(name=name, val=quantizing.pretty_int(val))
            else:
    ...
    def log_weights_sparsity(self, model, epoch):
        t, total = quantizing.weights_sparsity_tbl_summary(model, return_total_sparsity=True)
        self.pylogger.info("\nParameters:\n" + str(t))
    ...
    def log_model_buffers(self, model, buffer_names, tag_prefix, epoch, completed, total, freq):
        ...
        for n, m in model.named_modules():
            for buffer_name in buffer_names:
                ...
                data.append([quantizing.normalize_module_name(n) + '.' + buffer_name, *values])
        ...
        with ExitStack() as stack:
            ...
            for n, m in model.named_modules():
                for buffer_name in buffer_names:
                    ...
                    writer.writerow([quantizing.normalize_module_name(n) + '.' + buffer_name,
