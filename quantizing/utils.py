...
def find_module_by_fq_name(model, fq_mod_name):
    ...
    for module in model.modules():
        if hasattr(module, 'quantizing_name') and fq_mod_name == module.quantizing_name:
            return module
    return None
...
def sparsity_3D(tensor):
    ...
    if tensor.dim() != 4:
        return 0
    l1_norms = quantizing.norms.filters_lp_norm(tensor, p=1, length_normalized=False)
...
def non_zero_channels(tensor):
    ...
    norms = quantizing.norms.channels_lp_norm(tensor, p=1)
...
def norm_filters(weights, p=1):
    return quantizing.norms.filters_lp_norm(weights, p)
...
def _validate_input_shape(dataset, input_shape):
    if dataset:
        try:
            return tuple(quantizing.apputils.classification_get_input_shape(dataset))
        except ValueError:
